...
def test_coherency_scales_with_common_gain_power() -> None:
    az = np.deg2rad(np.array([15.0, 90.0, 210.0]))
    za = np.deg2rad(np.array([10.0, 35.0, 60.0]))
    gain = 2.0 - 0.5j

    reference = np.asarray(coherency(az_rad=az, za_rad=za))
    scaled = np.asarray(
        coherency(
            az_rad=az,
            za_rad=za,
...
def test_zero_gains_produce_zero_coherency() -> None:
    az = np.deg2rad(np.array([15.0, 90.0, 210.0]))
    za = np.deg2rad(np.array([10.0, 35.0, 60.0]))
